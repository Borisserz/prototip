...
const PRES_STAGES = [
  ...
  { id: 'analyze',  label: 'AI Анализ',           icon: <BrainCircuit className="w-4 h-4" />,  detail: 'Извлечение тем, метаданных, ключевых вопросов...' },
...
const DASH_STAGES = [
  { id: 'upload',   label: 'Загрузка файла',     icon: <UploadCloud className="w-4 h-4" />,      detail: 'Передача PDF на сервер...' },
  { id: 'extract',  label: 'Извлечение текста',  icon: <FileText className="w-4 h-4" />,          detail: 'Парсинг страниц, распознавание структуры...' },
  { id: 'analyze',  label: 'AI Анализ',           icon: <BrainCircuit className="w-4 h-4" />,     detail: 'Определение тем, KPI-метрик...' },
      ...
      <div className="px-5 py-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 pt-1">
          {[
            ...
            { label: result.output_type === 'presentation' ? 'Слайдов' : 'Графиков', value: result.slides?.length || '—', icon: Layers },
            { label: 'Время генерации', value: formatElapsed(elapsed), icon: Timer },
...
export const PDFGenerationHub: React.FC<PDFGenerationHubProps> = ({ token }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      } else if (outputType === 'dashboard' && data.dashboard_data) {
        addDashboard({
          id: `${id}_dash`,
          title: data.doc_title || files[0].name,
          timestamp: Date.now(),
          data: JSON.stringify(data.dashboard_data),
        });
      }
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="flex items-center gap-2 px-2 py-2 border-b border-white/5 flex-shrink-0">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> Назад к генерации
          </button>
        </div>
          ...
          {step === 'upload' && (
            ...
            >
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <div className="flex items-center gap-1.5 text-[11px] text-slate-600">
                        <Cpu className="w-3 h-3" /> AI-анализ
                      </div>
                      <div className="flex items-center gap-1.5 text-[11px] text-slate-600">
              ...
              {files.length > 0 && (
                ...
                >
                  Выбрать тип генерации <ChevronRight className="w-4 h-4 ml-1.5" />
                </Button>
              )}
            ...
            >
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-bold text-white">Выберите тип генерации</h3>
                ...
                {[
                  {
                    ...
                    features: ['AI-слайды из PDF', 'Графики ClickHouse', 'Экспорт .pptx', 'Онлайн-просмотр'],
                  },
                  {
              ...
              <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-slate-900/50 border border-white/5">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  PDF будет проиндексирован в базу знаний. AI извлечёт ключевые темы и
          ...
          {step === 'progress' && (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-white">AI обрабатывает документ...</p>
                  <p className="text-xs text-slate-500 truncate">{files[0]?.name}</p>
                </div>
              ...
              {error && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-rose-300 font-medium">Ошибка генерации</p>
              ...
              {!error && (
                <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-violet-500/5 border border-violet-500/10">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">
                    AI анализирует документ, формирует запросы к ClickHouse и генерирует
