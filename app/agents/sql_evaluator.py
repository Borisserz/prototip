...
{cfg.rules}"""
    ...
    def evaluate(self, question: str, sql: str, schema: str, sample_data: list[dict] = None) -> EvalDecision:
...
"""
        try:
            ...
            decision = call_structured(prompt, EvalDecision, system=self.system_prompt, agent_name="sql_evaluator")
            logger.info(f"SQL Eval Result: correct={decision.is_correct}, feedback={decision.feedback}")
            return decision
        except Exception as e:
            logger.error(f"Ошибка SqlEvaluatorAgent: {e}")
            return EvalDecision(is_correct=True, feedback="Fallback: Пропущена проверка из-за ошибки LLM.")
