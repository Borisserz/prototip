...
{cfg.rules}"""
    def evaluate(self, question: str, analysis: dict, raw_data: list[dict] = None) -> ReviewDecision:
        if not analysis:
...
"""
        try:
            ...
            decision = call_structured(prompt, ReviewDecision, system=self.system_prompt, agent_name=self.name)
