    ...
    try:
        ...
        for table in tables:
            ...
            for col_name, col_type, col_comment in columns_data:
                ...
                if 'String' in col_type and not any(x in col_name.lower() for x in ['id', 'inn', 'json', 'embedding', 'content']):
                    try:
                        enum_query = f"SELECT DISTINCT {col_name} FROM {table} LIMIT 10"
                        enum_vals = [row[0] for row in client.query(enum_query).result_rows if row[0]]
                        if enum_vals and len(enum_vals) <= 10:
                            col_info["enum_values"] = enum_vals
                            col_info["description"] += f" Возможные значения: {', '.join(enum_vals)}."
                    except Exception:
                        pass
        ...
        with open(out_file, 'w', encoding='utf-8') as f:
            yaml.dump(semantic_model, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
