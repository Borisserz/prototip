#!/usr/bin/env python3
"""Сборка изолированного клиента (Multi-tenant B2B).
...
def connect_pg(dsn: str):
    try:
        ...
        # резервный вариант на чистый pg8000
...
def render_compose(client_id: str, ch_port: int, password: str, native_port: int) -> str:
    return f"""# Настроено build_tenant.py для клиента '{client_id}'
services:
...
def index_semantics(semantics: list[dict], collection: str, ch_client_obj) -> str:
    ...
    резервный вариант). Возвращает строку-резюме. Полностью устойчив к отсутствию модели.
    """
    try:
...
# ─── основной поток ─────────────────────────────────────────────────────────────
def main() -> int:
    ap = argparse.ArgumentParser(description="Сборка изолированного B2B-клиента")
    ...
    tenant = tenant_store.create_tenant(
        ...
        enforce_client_id=args.add_client_id,
        client_id_value=cid,
    )
