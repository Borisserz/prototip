...
TAX_TYPES = ["НДС", "Налог на прибыль", "Подоходный налог", "Налог на недвижимость", "Земельный налог", "Акцизы"]
REGIONS = ["Минск", "Гомельская область", "Брестская область", "Витебская область", "Могилевская область", "Гродненская область", "Минская область"]
...
def get_clickhouse_ddl():
    """Возвращает DDL схему для ClickHouse, соответствующую сгенерированному датасету.
    Используем лучшие практики: LowCardinality для строк-перечислений, MergeTree с правильным ORDER BY.
    """
    return """
...
def generate_db(count=50000):
    ...
    with open(DB_FILE, "w", newline='', encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["id", "taxpayer_id", "taxpayer_type", "industry", "region", "tax_type", "accrued", "paid", "debt", "penalty", "status", "period"])
        ...
        for i in range(1, count + 1):
            ...
            writer.writerow([
                i, taxpayer_id, taxpayer_type, industry, region, tax_type, 
                accrued, paid, debt, penalty, status, period_str
            ])
            
            if i % 10000 == 0:
                print(f"Сгенерировано {i} строк...")
                
    print(f"Генерация завершена. Файл сохранен: {DB_FILE} (Размер: {os.path.getsize(DB_FILE) / (1024*1024):.2f} MB)")
