...
def discover_schema(host="localhost", port=8123, output_path="data/semantic_model.yaml"):
    ...
    semantic_model = {"version": "1.0", "models": []}

    for table in tables:
        logger.info(f"Scanning table: {table}")
        model_desc = {
            "name": table,
            "description": f"Таблица {table}. Автоматически обработано (Schema Discovery).",
            "columns": [],
        }
        ...
        for row in cols_res.result_rows:
            ...
            col_desc = {"name": col_name, "type": col_type, "description": "Описание отсутствует."}
            ...
            if "String" in col_type:
                ...
                try:
                    ...
                    json_check = client.query(
                        f"SELECT {col_name} FROM {table} WHERE {col_name} LIKE '{{%}}' LIMIT 1"
                    )
                        ...
                        try:
                            ...
                            if isinstance(parsed, dict):
                                ...
                                col_desc["description"] = (
                                    f"JSON поле. Содержит ключи: {', '.join(keys)}. Для доступа используйте JSONExtractString({col_name}, 'key')."
                                )
                ...
                if not col_desc.get("is_json"):
                    try:
                        unique_res = client.query(
                            f"SELECT DISTINCT {col_name} FROM {table} LIMIT 15"
                        )
                        unique_vals = [
                            str(r[0])
                            for r in unique_res.result_rows
                        ...
                        if 0 < len(unique_vals) <= 12:
                            ...
                            col_desc["description"] = (
                                f"Возможные значения: {', '.join(unique_vals)}."
                            )
                    except Exception as e:
