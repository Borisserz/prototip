...
поэтому запись через этот модуль автоматически подхватывается агентами при
    ...
    def load_all(self, force: bool = False) -> dict[str, Any]:
        with self._lock:
            ...
            if force or mtime > self._last_mtime or not self._cache:
                ...
                logger.info("PromptStore: перечитан %s (%d агентов)", self.config_path, len(self._cache))
            return self._cache
