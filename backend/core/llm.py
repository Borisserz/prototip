...
Все LLM-вызовы в проекте — ТОЛЬКО через Модуль (structured JSON по Pydantic, temperature=0).
...
# Centralized logging setup for  (called once on first import of core.llm, which agents use)
_log_configured = False


def setup_logging() -> None:
    """Настроить логгер ОДИН раз: stdout + out/run.log, INFO (Structured JSON )."""
...
# бизнес-метрики Prometheus (импорт защищён — не должен ломать LLM-слой)
try:
...
def _chat_once(
    *, model: str, messages: list[dict], schema: type[T], agent_name: str = "unknown"
) -> T:
    ...
    if use_vertex:
        ...
        used_model = os.getenv("AI_MODEL", "gemini-3.5-flash")
        ...
        resp = client.models.generate_content(
            model=used_model, contents=genai_messages, config=config
        )
    ...
    # Логируем в ClickHouse и Prometheus (унифицировано)
    try:
        from app.agent_context import get_user_role
        from app.utils.system_logger import audit_logger

        role_val = get_user_role()
        user_role = role_val if role_val else "system"
...
def call_structured(
    ...
    for attempt in range(1, LLM_MAX_RETRIES + 1):
        try:
            ...
            try:
                return _chat_once(
                    model=mdl, messages=repair_messages, schema=schema, agent_name=agent_name
                )
        ...
        if attempt < LLM_MAX_RETRIES:
            ...
            if "429" in str(last_error) or "RESOURCE_EXHAUSTED" in str(last_error):
                delay = 5.0 * attempt
                _llm_logger.warning(
                    f"[LLM] Rate limit hit. Sleeping for {delay}s before attempt {attempt + 1}"
                )
                time.sleep(delay)
            else:
