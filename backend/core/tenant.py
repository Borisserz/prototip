...
class TenantStore:
    ...
    def _from_dict(item: dict[str, Any]) -> Tenant:
        ...
        return Tenant(
            ...
            # (поля опциональны — старые реестры читаются без ошибок)
    ...
    def update_tenant(self, client_id: str, **fields: Any) -> Tenant | None:
        ...
        with self._lock:
            ...
            # обновление клиентского Postgres DSN (шифруется at-rest)
            if "pg_dsn" in fields:
    ...
    def get_pg_dsn(self, tenant: Tenant) -> str:
        ...
        try:
            ...
            logger.error(
                "TenantStore: не удалось расшифровать pg_dsn для %s: %s", tenant.client_id, e
            )
            return ""
