"""Долгосрочная память пользователя: профили + RAG по истории чата.

Архитектурно повторяет существующий RAG (app/services/rag_service.py): хранилище —
ClickHouse, эмбеддинги — all-MiniLM-L6-v2 (с резервный вариант на FakeEmbeddings), поиск —
    ...
    def search_recent(
        ...
        try:
            ...
            if vec:
                ...
                # резервный вариант без эмбеддингов — просто последние запросы за период
                sql = f"""
            ...
            return [
                {"prompt": r[0], "response": r[1], "ts": r[2], "dist": float(r[3])} for r in rows
            ]
        except Exception as e:  # noqa: BLE001
