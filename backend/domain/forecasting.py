"""Движок прогнозирования (Phase 3, предиктивная аналитика).
...
описание делает уже LLM-агент (ForecastAnalystAgent), а не этот модуль.
"""
...
def forecast_series(
    ...
    if n < 2:
        ...
        return ForecastResult("constant", horizon, hist_labels, y.tolist(), pts, {}, False, warnings)
...
def detect_time_value_columns(rows: list[dict]) -> tuple[str | None, str | None]:
    ...
    time_kw = ("period", "date", "month", "year", "quarter", "день", "дата", "месяц", "год", "период")
