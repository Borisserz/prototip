...
def test_data_agent_basic_mock() -> None:
    ...
    fake_data = [
        {"region": "г. Минск", "d": 298775721.0},
        {"region": "Гродненская область", "d": 213597479.0},
        {"region": "Минская область", "d": 157163460.0},
        {"region": "Гомельская область", "d": 47971886.0},
        {"region": "Витебская область", "d": 11076131.0},
    ]

    with (
        patch("app.agents.data_agent.call_structured") as mock_call,
        patch.object(agent, "_execute_sql", return_value=fake_data),
        patch("app.agents.data_agent.SqlEvaluatorAgent") as mock_eval_cls,
    ):
        mock_call.return_value = type(
            "obj", (object,), {"sql": fake_sql, "step_by_step_reasoning": "mock"}
        )()
        mock_eval_cls.return_value.evaluate.return_value = type(
            "ev", (object,), {"is_correct": True, "feedback": "ok"}
        )()
...
def test_data_agent_rejects_write() -> None:
    ...
    with patch("app.agents.data_agent.call_structured") as mock_call:
        mock_call.return_value = type(
            "obj", (object,), {"sql": bad_sql, "step_by_step_reasoning": "mock"}
        )()
...
def test_data_agent_self_correction_on_error() -> None:
    ...
    fake_data = [
        {"region": "г. Минск", "c": 12},
        {"region": "Минская область", "c": 8},
        {"region": "Брестская область", "c": 5},
    ]
    ...
    def fake_call(prompt, schema, **kw):
        ...
        if len(calls) == 1:
            return type(
                ...
                (object,),
                {
                    "sql": "SELECT foo FROM default.enterprise_taxes LIMIT 3",
                    "step_by_step_reasoning": "mock",
                },
            )()
        return type(
            "obj",
            (object,),
            {
                "sql": "SELECT region, COUNT(*) as c FROM default.enterprise_taxes GROUP BY region LIMIT 3",
                "step_by_step_reasoning": "mock",
            },
        )()
    ...
    def fake_execute(sql):
        lowered = sql.lower()
        if "foo" in lowered:
            raise ValueError("column foo does not exist")
        return fake_data

    with (
        patch("app.agents.data_agent.call_structured", side_effect=fake_call),
        patch.object(agent, "_execute_sql", side_effect=fake_execute),
        patch("app.agents.data_agent.SqlEvaluatorAgent") as mock_eval_cls,
    ):
        mock_eval_cls.return_value.evaluate.return_value = type(
            "ev", (object,), {"is_correct": True, "feedback": "ok"}
...
def test_data_agent_live_5_questions() -> None:
    """Живой прогон на 5 типовых вопросах (как требует spec 'Готово')."""
    agent = DataAgent()
    questions = [
