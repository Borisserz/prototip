...
@pytest.mark.skipif(
    ...
    reason="Ollama + qwen2.5-coder:7b-instruct недоступен для live теста",
)
...
def test_presentation_slide_header_uses_action_title() -> None:
    ...
    with (
        ...
        agent.run(
            ["Какая задолженность по регионам?"], include_title=False, include_recommendations=False
        )
