...
def test_regular_user(auth):
    # Обычный пользователь user1 (роль manager), без привязки к блоку.
    u = auth.authenticate_user("user1", "user123")
    assert u is not None and u["role"] == "manager"
    # Удалённые демо-пользователи больше не аутентифицируются.
    assert auth.authenticate_user("grodno", "grodno123") is None
    assert auth.authenticate_user("minsk", "minsk123") is None
