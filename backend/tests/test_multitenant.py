"""Тесты: SQL-guard, реестр клиентов, build-утилиты."""
...
def test_extra_region_filter_applied():
    out = sql_guard.secure_sql(
        "SELECT paid FROM tax_data", tenant=None, extra_filters={"region": "г. Минск"}
    )
    assert "region = 'г. Минск'" in out


# ─── Per-tenant user: персональные allowed_tables / allowed_columns ───────────
def test_user_allowed_columns_pass():
    out = sql_guard.secure_sql(
        "SELECT region, paid FROM tax_data",
        tenant=None,
        allowed_columns=["region", "paid"],
    )
    assert "region" in out and "paid" in out


def test_user_disallowed_column_blocked():
    with pytest.raises(sql_guard.SqlSecurityError):
        sql_guard.secure_sql(
            "SELECT debt FROM tax_data",
            tenant=None,
            allowed_columns=["region", "paid"],
        )


def test_user_select_star_blocked_with_columns():
    with pytest.raises(sql_guard.SqlSecurityError):
        sql_guard.secure_sql(
...
def test_user_count_star_allowed_with_columns():
    out = sql_guard.secure_sql(
        "SELECT region, count(*) AS c FROM tax_data GROUP BY region",
        tenant=None,
        allowed_columns=["region"],
    )
...
def test_user_allowed_tables_override_blocks():
    with pytest.raises(sql_guard.SqlSecurityError):
        sql_guard.secure_sql(
            "SELECT region FROM tax_data",
            tenant=None,
            allowed_tables=["sales"],
        )


def test_user_injected_client_id_column_allowed():
    # client_id добавляется системой и не должен отклоняться проверкой колонок
    out = sql_guard.secure_sql(
        "SELECT region FROM tax_data",
        tenant=_T(),
        allowed_columns=["region"],
...
def test_build_tenant_helpers():
    ...
    spec = importlib.util.spec_from_file_location(
        "build_tenant", ROOT / "scripts" / "build_tenant.py"
    )
    ...
    assert (
        "client_id String" in ddl
        and "ORDER BY client_id" in ddl
        and "amount Nullable(Float64)" in ddl
    )
