...
def _ask(
    *,
    ...
    return AskResult(
        ...
        sql="SELECT 1",
        data=data or [{"region": "г. Минск", "debt": 100}, {"region": "Гомельская область", "debt": 200}],
...
def test_choose_layout_router():
    ...
    assert r.choose_layout("scatter") == LayoutKind.TOP_BOTTOM
...
def test_chart_table_layout_adds_table(tmp_path):
    ...
    renderer._render_chart_and_table_layout(slide, res, None, "horizontal_bar")
...
def test_summary_slide_kpi_cards(tmp_path):
    ...
    texts = "".join(sh.text for sh in slide.shapes if hasattr(sh, "text"))
    assert "Суммарные начисления" in texts or "Регионов" in texts
    assert BADGE_TEXT in texts