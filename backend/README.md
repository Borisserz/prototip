...
| Слой | Где | Назначение |
|------|-----|-----------|
| **Веб / API** | `app/main.py`, `app/routers/`, `app/api/`, `app/auth.py`, `app/services/` | HTTP-маршруты: чат, дашборды, экспорт (PDF/DOCX/XLSX), рассылки, **админка клиентов** |
...
См. корневой `.env.example`. Ключевые: `CLICKHOUSE_*`, `MINIO_*`,
