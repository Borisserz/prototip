...
class DecompositionResult(BaseModel):
    is_complex: bool = Field(
        description="Требует ли вопрос разбиения на несколько независимых подзапросов к БД?"
    )
    tasks: list[str] = Field(
        description="Список подзадач на естественном языке. Если вопрос простой, здесь должен быть один элемент."
    )
...
"""
        try:
            ...
            res = call_structured(
                prompt, DecompositionResult, system=self.system_prompt, agent_name=self.name
            )
            if res.is_complex and res.tasks:
                logger.info(
                    f"Task Decomposer разбил задачу на {len(res.tasks)} частей: {res.tasks}"
                )
                return res.tasks
            else:
