"""PresentationAgent: авто-сборка .pptx презентации.
...
class PresentationAgent(BaseAgent):
    ...
    def _slug(self, text: str, max_len: int = 40) -> str:
        # транслитерация кириллицы → ASCII, чтобы избежать latin-1 ошибок в HTTP Content-Disposition
...
"""
    ...
    def _generate_placeholder_png(
        ...
        *,
        ...
        """Генерация PNG-превью слайда через Pillow (dark-тема, макет по типу слайда)."""
        try:
            import textwrap
            ...
            r, g, b = accent
            draw.rectangle(
                [W // 2, 0, W, 8], fill=(max(0, r - 40), max(0, g - 20), min(255, b + 40))
            )
            ...
            draw.text((122, 60), "Enterprise Analytics", fill=TEXT_D, font=f_tiny)
            ...
            draw.rounded_rectangle(
                [W - bw - 60, 28, W - 60, 84], radius=10, outline=accent, width=2
            )
            ...
            if slide_type == "title":
                ...
                sub = (
                    content_lines[0]
                    if content_lines
                    else "Синтетические данные · Республика Беларусь"
                )
                ...
                for val, lab, col in kpis:
                    draw.rounded_rectangle(
                        [W // 2 + 40, ky, W - 80, ky + 105], radius=14, fill=CARD
                    )
                    draw.rounded_rectangle(
                        [W // 2 + 40, ky, W // 2 + 52, ky + 105], radius=4, fill=col
                    )
                ...
                for i, theme in enumerate(content_lines[:6]):
                    ...
                    draw.rounded_rectangle(
                        [108, cy2 + ch2 // 2 - 26, 158, cy2 + ch2 // 2 + 26], radius=10, fill=col
                    )
                ...
                for i, tw_text in enumerate(content_lines[:6]):
                    ...
                    draw.multiline_text(
                        (172, cy2 + 14), wrapped, fill=TEXT_W, font=f_body, spacing=8
                    )
                    pct = 85 - i * 8
                    by = cy2 + ch2 - 14
                    draw.rounded_rectangle([172, by, W - 96, by + 6], radius=3, fill=SURFACE)
                    draw.rounded_rectangle(
                        [172, by, 172 + int((W - 96 - 172) * pct / 100), by + 6], radius=3, fill=col
                    )
                ...
                for i, rec in enumerate(content_lines[:4]):
                    ...
                    draw.multiline_text(
                        (rx + 16, ry + 56), wrapped, fill=TEXT_W, font=f_body, spacing=8
                    )
            ...
            draw.text(
                (80, H - 42),
                "Prototip BI  •  Analytics Platform  •  Синтетические данные (демо)",
                fill=TEXT_D,
                font=f_tiny,
            )
        ...
        for idx, (q, res) in enumerate(zip(questions, results)):  # noqa: B905
            ...
            if res.chart_spec and res.data:
                try:
                    ...
                    slide_png = (
                        out_dir / f"pres_slide_{presentation_id}_{idx}_{self._slug(header)}.png"
                    )
    ...
    def run(
        ...
        # генерация PNG для каждого слайда
    ...
    def _build_pptx(
        self, state: PresentationState, slide_png_paths: list[str | None], out_dir: Path
    ) -> tuple[Presentation, list[str], list[SlideData]]:
        prs = Presentation()
        ...
        renderer.create_summary_slide(
            ...
            state.narrative.overview,
            state.results[0] if state.results else None,
        )
        final_png_paths.append("")
        renderer.create_themes_slide(
            prs.slides.add_slide(blank), state.narrative.themes, state.questions
        )
        final_png_paths.append("")
        ...
        for idx, (q, res) in enumerate(zip(state.questions, state.results)):  # noqa: B905
            ...
            renderer.render_question_slide(
                slide, q, res, state.prefs.get(idx), slide_png_paths[idx]
            )
        ...
        if state.include_recommendations:
            renderer.create_recommendations_slide(
                prs.slides.add_slide(blank), state.narrative.recommendations
            )
            final_png_paths.append("")

        target = (
            state.num_slides
            if (state.num_slides is not None and state.num_slides > 0)
            else len(prs.slides)
        )
        ...
        # маппинг slide_idx → (type, title, content) для thumbnail-рендера
        slide_type_labels: list[tuple[str, str, str | list[str] | None]] = []
        _si = 0
        if state.include_title:
            slide_type_labels.append(
                (
                    "title",
        ...
        if state.include_recommendations:
            slide_type_labels.append(
                ("recommendations", "Рекомендации", state.narrative.recommendations)
            )
            _si += 1
        while _si < len(final_png_paths):
            slide_type_labels.append(
                ("appendix", "Приложение", "Дополнительные материалы и методология расчетов")
            )
            _si += 1
        ...
        for slide_idx, (stype, stitle, scontent) in enumerate(slide_type_labels):
            slides_data.append(
                SlideData(slide_idx=slide_idx, slide_type=stype, title=stitle, content=scontent)
            )
    ...
    def update_presentation(
        self, presentation_id: str, slide_updates: dict[int, SlideUpdate]
    ) -> PresentationResult:
        out_dir = Path("out")
        ...
        # маппинг slide_idx → (тип, индекс в соотв. списке)
        _si = 0
        slide_idx_map: dict[int, tuple[str, int]] = {}
        if state.include_title:
        ...
        # какие chart-слайды нужно перерендерить с новым типом
        chart_type_overrides: dict[int, str] = {}

        # применяем изменения к стейту
        for slide_idx_str, update in slide_updates.items():
            ...
            if stype == "summary":
                ...
                if update.chart_type is not None:
                    ...
                    # сохраняем предпочтение типа графика в prefs
                    state.prefs[sref] = update.chart_type
            elif stype == "takeaways":
                if update.content is not None:
                    state.narrative.key_takeaways = (
                        update.content
                        if isinstance(update.content, list)
                        else [str(update.content)]
                    )
                ...
                if update.content is not None:
                    state.narrative.recommendations = (
                        update.content
                        if isinstance(update.content, list)
                        else [str(update.content)]
                    )
        ...
        for idx, q in enumerate(state.questions):
            if idx in chart_type_overrides:
                ...
                try:
                    ...
                    if res.chart_spec and res.data:
                        ...
                        with suppress(Exception):
                            fig.update_layout(title=dict(text=""))
                        header = (
                            getattr(slide_spec, "action_title", None) or slide_spec.title or q[:30]
                        )
                        new_png_path = (
                            out_dir
                            / f"pres_slide_{presentation_id}_{idx}_{self._slug(header)}_{new_type}.png"
                        )
                        export_png(fig, new_png_path, scale=2.0)
                        slide_png_paths.append(
                            str(new_png_path.resolve()) if new_png_path.exists() else None
                        )
                        logger.info(f"[update_presentation] regenerated chart {idx} as {new_type}")
                    else:
                        # чарт-спек отсутствует — оставляем старый PNG
                ...
                res = state.results[idx]
                header = (
                    getattr(res.chart_spec, "action_title", None)
                    or getattr(res.chart_spec, "title", None)
                    or q
                )
                expected_png = (
                    out_dir / f"pres_slide_{presentation_id}_{idx}_{self._slug(header)}.png"
