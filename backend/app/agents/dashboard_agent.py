"""DashboardAgent: вопрос на русском → комплексный дашборд (KPI + графики + аналитика)."""
...
class _DashboardComposition(BaseModel):
    ...
    charts: list[ChartSpec] = []  # резервный вариант если нет идей
    chart_ideas: list[
        str
...
class DashboardAgent(BaseAgent):
    ...
    description = "По вопросу строит дашборд: вызывает Data/Analyst/Chart (reuse), получает 3-5 ChartSpec + KPI + layout + insights + reasoning. Spec-first, безопасное игнорирование ошибок."
    ...
    def run(self, request: DashboardRequest) -> DashboardResult:
        ...
        if not data:
            try:
                ...
                emit_pipeline_stage(
                    "sql", "running", "Загрузка данных для дашборда...", agent="dashboard_agent"
                )
        ...
        # RAG-контекст для шаблонов дашборда
        rag_context = ""
        try:
...
- ВАЖНО: Обязательно сгенерируй МИНИМУМ 3 chart_ideas! Дашборд с 1 графиком недопустим.
...
- (опционально charts как резервный вариант)
...
"""
        ...
        try:
            composition = call_structured(
                prompt, schema=_DashboardComposition, system=system_msg, agent_name=self.name
            )
            logger.info(
        ...
        # 4. Пост-обработка и ограничения + активация реального reuse ChartAgent (Parallel - )
        ...
        try:
            ...
            def _generate_chart(idea):
                ...
                return [repair_chart_spec(spec, data, question=idea_str) for spec in cr.specs]

            with concurrent.futures.ThreadPoolExecutor(max_workers=max_c) as pool:
                futures = {}
                for idea in (chart_ideas or [])[:max_c]:
                    ctx = contextvars.copy_context()
                    futures[pool.submit(ctx.run, _generate_chart, idea)] = idea

                for future in concurrent.futures.as_completed(futures):
                    try:
                        charts.extend(future.result())
                    except Exception as e:
        ...
        if not charts and composition.charts:
            charts = [repair_chart_spec(c, data, question=q) for c in composition.charts[:max_c]]
    ...
    def _fallback_result(
        ...
        return DashboardResult(
            ...
            data=data or [],
            source_sql=source_sql,
            reasoning=f"резервный вариант из-за ошибки LLM: {error[:200]}",
        )
