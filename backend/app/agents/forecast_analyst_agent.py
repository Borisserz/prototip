"""ForecastAnalystAgent: узел предиктивной аналитики.
...
class ForecastAnalystResult(AgentResult):
    ...
    data: list[dict[str, Any]] = Field(
        default_factory=list, description="История+прогноз для графика"
    )


class _Narrative(BaseModel):
    narrative: str = Field(
        ..., description="Деловое описание прогноза на русском (2-4 предложения)"
    )
...
class ForecastAnalystAgent(BaseAgent):
    ...
    def _summarize(self, question: str, result: fc.ForecastResult, t_col: str, v_col: str) -> str:
        ...
        try:
            ...
            res = call_structured(prompt, schema=_Narrative, system=system, agent_name=self.name)
