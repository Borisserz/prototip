"""ReportDocxAgent: сборка отчёта в формате Word (.docx).
...
Назначение:
  ...
  - готовый .docx зеркалится в MinIO бакет `documents`;
...
class ReportDocxInput(BaseModel):
    ...
    question: str | None = Field(
        None, description="Запрос пользователя (если markdown нужно сгенерировать)"
    )
...
class ReportDocxAgent(BaseAgent):
    ...
    def _coerce_input(self, request: Any, kwargs: dict) -> ReportDocxInput:
        ...
        if isinstance(request, str):
            return ReportDocxInput(
                markdown=request,
                **{k: v for k, v in kwargs.items() if k in ReportDocxInput.model_fields},
            )
    ...
    def _generate_markdown(self, inp: ReportDocxInput) -> tuple[str, str | None]:
        ...
        try:
            ...
            logger.info("ReportDocxAgent: LLM недоступна (%s), сборка резервный вариант-отчёта", e)
