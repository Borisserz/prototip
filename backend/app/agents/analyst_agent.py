"""AnalystAgent: вопрос + данные (из DataAgent) → AnalysisResult.
    ...
    def _format_data_context(
        ...
        *,
        ...
        if drilldown_filters:
            ...
            parts.append(f"Активные фильтры детализация: {filters_ru}.")
        if source_sql:
    ...
    def run(
        ...
        if not data:
            ...
            return AnalysisResult(
                ...
                success=False,
                degraded=True,
                reasoning="Fallback: передан пустой data. Выводы минимальны, чтобы процесс не упал.",
            )
...
"""
        ...
        try:
            ...
            analysis = call_structured(
                prompt, schema=AnalysisResult, system=system_msg, agent_name=self.name
            )
        except Exception as e:
