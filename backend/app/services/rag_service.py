...
def get_embeddings_model():
    try:
        ...
        logger.warning(
            "langchain_huggingface не установлен — используется FakeEmbeddings, RAG-векторы будут случайными."
        )
        if os.getenv("APP_ENV", "development").lower() == "production":
            raise RuntimeError(
                "FakeEmbeddings cannot be used in production! Please install langchain_huggingface."
            )
...
def initialize_rag(docs_directory: str = None):
    logger.info("Инициализация RAG процесс в ClickHouse...")
    init_tables()
    ...
    if docs_directory and os.path.exists(docs_directory):
        ...
        txt_loader_fallback = DirectoryLoader(
    ...
    for split in splits:
        ...
        data.append(
            [uuid.uuid4().hex, split.metadata.get("source", "unknown"), split.page_content, emb]
        )
...
def ingest_document(file_path: str) -> bool:
            ...
            with pdfplumber.open(file_path) as pdf:
                for i, page in enumerate(pdf.pages):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Ошибка извлечения таблиц на странице {i} (возможно скан): {e}"
                        )
    ...
    for split in splits:
        emb = embeddings_model.embed_query(split.page_content)
        data.append([uuid.uuid4().hex, file_path, split.page_content, emb])
...
def get_rag_context(query: str, k: int = 4) -> str:
    ...
    try:
        sql = """
        SELECT content, cosineDistance(embedding, %(vector)s) as dist 
        FROM default.knowledge_base 
        ORDER BY dist ASC 
        LIMIT %(k)s
        """
        result = ch_client.get_client().query(sql, parameters={"vector": query_vector, "k": int(k)})
        docs = []
...
def delete_knowledge_document(source: str) -> bool:
    try:
        sql = "ALTER TABLE default.knowledge_base DELETE WHERE source = %(source)s"
        ch_client.command(sql, parameters={"source": str(source)})
        return True
    except Exception as e:
...
def initialize_dashboard_rag():
    ...
    if data:
        ch_client.insert(
            "dashboard_knowledge",
            data,
            column_names=["id", "title", "description", "full_json", "embedding"],
        )
        return True
    return False
...
def search_dashboards(query: str, k: int = 1):
    ...
    try:
        sql = """
        SELECT full_json, cosineDistance(embedding, %(vector)s) as dist 
