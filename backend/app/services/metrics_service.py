"""Агрегация системных метрик для страницы «Мониторинг».
...
агрегаты для нативной страницы мониторинга на ``recharts`` через маршрут
...
def _demo(hours: int, step: int) -> dict:
        ...
        by_agent.append(
            {
                "agent": a,
                "calls": c,
                "avg_latency_ms": round(rnd.uniform(600, 1600), 1),
                "prompt_tokens": pt,
    ...
    by_model = [
        {
            "model": models[0],
            "calls": int(total_calls * 0.7),
            "tokens": int(total_calls * 0.7 * 540),
            "avg_latency_ms": 980.0,
        },
        {
            "model": models[1],
            "calls": int(total_calls * 0.3),
            "tokens": int(total_calls * 0.3 * 610),
            "avg_latency_ms": 740.0,
        },
    ]
