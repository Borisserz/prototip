"""
FastAPI entrypoint. WebSocket-сессии, фоновые задачи, LangGraph-граф.
"""
...
# rate-limiting: глобальный дефолт + строгие лимиты на login и LLM-вызовы
# ключ — IP клиента (за прокси нужен X-Forwarded-For)
...
try:
    ...
    _original_get_route_name = _routing.get_route_name
    ...
    _routing.get_route_name = _safe_get_route_name
    Instrumentator().instrument(app).expose(app)
except Exception as e:
    logger.error(f"Failed to initialize Prometheus Instrumentator: {e}")

# регистрируем кастомные бизнес-метрики (LLM, SQL, узлы LangGraph),
# чтобы они присутствовали в /metrics ещё до первого запроса.
try:
    import app.observability as _observability  # noqa: F401  # alias, чтобы не затереть FastAPI-инстанс `app`
...
async def lifespan(app: FastAPI):
    ...
    # таблицы долгосрочной памяти (профили + история чата)
    try:
...
# CORS: origins берём из переменной окружения, wildcard "*" + credentials недопустим
_cors_origins = [
    o.strip()
...
# ETL-оркестрация (Airflow) — кнопочная инициализация клиентов
...
def _require_feature(user: dict, feature: str, label: str) -> None:
    """Гейт по фиче-праву пользователя блока (can_dashboard / can_presentation).

    Разрешает безусловно: admin, глобальных пользователей и вход клиента по api_key
    (нет записи в реестре per-tenant). Запрещает только пользователю блока с явным
    отключённым правом.
    """
    ...
    try:
        ...
        perms = get_user_permissions(client_id, username)
    except Exception:  # noqa: BLE001
        perms = None
    if perms is None:
        return
    if not perms.get(feature, True):
        raise HTTPException(403, f"У вашего пользователя нет права на {label}.")
...
def readyz() -> JSONResponse:
    ...
    # Векторное хранилище знаний (RAG) работает поверх ClickHouse (cosineDistance),
    # поэтому отдельный readiness-чек Qdrant не нужен — сервис удалён из стека.
...
def _set_auth_cookie(response: Response, token: str) -> None:
    """Кладёт access-токен в httpOnly+SameSite cookie.

    Secure включается вне dev, чтобы cookie ходила только по HTTPS.
    """
...
def login(request: Request, form_data: LoginRequest, response: Response):
    ...
    role = user["role"]
    # per-tenant: добавляем client_id в токен, чтобы оркестратор знал блок
    ...
    # дублируем токен в httpOnly-cookie; тело ответа оставлено для совместимости с фронтом
...
def logout(response: Response):
    """Чистит httpOnly-cookie с токеном."""
    from app.auth import COOKIE_NAME
...
def ask_endpoint(
    request: Request, payload: AskRequest, user: dict = Depends(get_current_user)
) -> dict[str, Any]:
    """Универсальный запрос через PlannerAgent (Обычный JSON-ответ)."""
    cid = new_correlation_id()
    res = get_orchestrator().ask(
        payload.question, drilldown=payload.drilldown, correlation_id=cid, user=user
    )
...
async def chat_websocket(websocket: WebSocket, token: str | None = Query(default=None)):
    # аутентификация ДО accept() — токен из query-параметра, cookie или Authorization-заголовка
    ...
    try:
        while True:
            ...
            # drilldown-контекст из фронтенда
            drilldown_ctx = None
            ...
            if drilldown_raw and isinstance(drilldown_raw, dict):
                ...
                if key and value:
                    # маппинг полей графика → имена SQL-колонок
                    key_mapping = {
                        "region": "region",
                        "REGION": "region",
                        "Регион": "region",
            ...
            await websocket.send_text(
                json.dumps({"type": "status", "content": "Изучаем ваш запрос..."})
            )
            await asyncio.sleep(0.5)
            await websocket.send_text(
                json.dumps({"type": "status", "content": "Анализ данных и подготовка ответа..."})
            )
            ...
            def run_ask():
                ...
                with debate_context(debate_q):
                    # пробрасываем role/client_id в оркестратор — WS проходит те же проверки что и REST
            ...
            # поллинг очереди debate + progress пока задача не завершилась
            ...
            while not ask_task.done():
                ...
                # отражаем прогресс pipeline через WS
            ...
            if ask_result.charts:
                try:
                    js.loads(content)  # если ответ чистый JSON — оборачиваем
            ...
            # Добавляем SQL в ответ для Режима Аналитика
...
async def upload_data(file: UploadFile = File(...)):
    ...
    return {
        "status": "success",
        "message": f"Файл {file.filename} загружен в dropzone и будет обработан ETL.",
    }
...
async def trigger_watcher(background_tasks: BackgroundTasks):
    ...
    return {
        "status": "accepted",
        "message": "Проактивное сканирование запущено в фоне. Ожидайте письмо.",
    }
...
def generate_dashboard(
    request: Request, payload: DashboardRequest, user: dict = Depends(get_current_user)
) -> DashboardResult:
    """Явный fast-path дашборда через Orchestrator.dashboard()."""
    _require_feature(user, "can_dashboard", "генерацию дашбордов")
    cid = new_correlation_id()
    drilldown = None
...
def generate_presentation(
    request: Request, payload: PresentationRequest, user: dict = Depends(get_current_user)
):
    ...
    _require_feature(user, "can_presentation", "генерацию презентаций")
    cid = new_correlation_id()
    questions = [q.text for q in payload.questions]

    # контекст аудитории и детальности для обогащения промпта
    audience_hint = {
    ...
    # преобразуем всё к строкам, чтобы оркестратор получил плоский список
    q_str: list[str] = []
    for q in (
        payload.questions
    ...
    # если executor поймал исключение — проверяем тип результата
    ...
    # JSONResponse мимо response_model — иначе pydantic обрежет bytes_path
    try:
    ...
    try:
        ...
        return {
            "kpi": [
                {
                    "title": "Начислено",
                    "value": f"{total_accrued / 1e6:,.1f}M бел. руб.",
                    "trend": "+5%",
                    "status": "good",
                },
                {
                    "title": "Оплачено",
                    "value": f"{total_paid / 1e6:,.1f}M бел. руб.",
                    "trend": "+2%",
                    "status": "neutral",
                },
                {
                    "title": "Задолженность",
                    "value": f"{total_debt / 1e6:,.1f}M бел. руб.",
                    "trend": "-1%",
                    "status": "good",
                },
                {
                    "title": "Уровень сборов",
                ...
                {
                    "title": "Ср. пеня",
                    "value": f"{avg_penalty:,.0f} бел. руб.",
                    "trend": "+12%",
                    "status": "bad",
                },
            ]
...
def export_custom_excel(payload: CustomExportRequest):
    """Экспорт текущей выборки данных в красивый Excel ."""
    import os
    ...
    try:
        ...
        return {
            "status": "ok",
            "url": f"http://127.0.0.1:8000/api/v1/download?file={os.path.basename(file_path)}",
        }
    except Exception as e:
...
class ReportDocxRequest(BaseModel):
    """Запрос на сборку отчёта в формате Word (.docx) — ."""
...
class ForecastRequest(BaseModel):
    """Запрос на прогноз временного ряда из данных дашборда ."""
...
class TenantCreateRequest(BaseModel):
    ...
    # подключение Postgres клиента + расписание ETL + лимит юзеров
    pg_dsn: str = ""
...
@app.post(
    "/api/v1/admin/tenants/{client_id}/rotate-token",
...
@app.delete(
    "/api/v1/admin/tenants/{client_id}", dependencies=[Depends(require_admin)], tags=["admin"]
)
...
@app.get(
    "/api/v1/admin/tenants/{client_id}/stats", dependencies=[Depends(require_admin)], tags=["admin"]
)
...
@app.get(
    "/api/v1/admin/tenants/{client_id}/impersonate",
    dependencies=[Depends(require_admin)],
    tags=["admin"],
)
...
class TenantUserCreateRequest(BaseModel):
    """Создание пользователя блока админом с гранулярными правами."""
    ...
    rls_filters: dict[str, list[str]] = Field(default_factory=dict)
    can_dashboard: bool = True
    can_presentation: bool = True


class TenantUserUpdateRequest(BaseModel):
    """Частичное обновление пользователя блока (все поля опциональны)."""
...
@app.get(
    "/api/v1/admin/tenants/{client_id}/users", dependencies=[Depends(require_admin)], tags=["admin"]
)
def list_tenant_users_endpoint(client_id: str):
    ...
    if not t:
        raise HTTPException(404, "Клиент не найден")
    users = tenant_users.list_users(client_id)
    return {
        "client_id": client_id,
        "users": users,
        "max_users": t.max_users,
        "active_count": tenant_users.count_active_users(client_id),
    }


@app.get(
    "/api/v1/admin/tenants/{client_id}/tables",
    dependencies=[Depends(require_admin)],
    tags=["admin"],
)
def list_tenant_tables_endpoint(client_id: str):
    """Доступные таблицы и колонки клиента (для назначения прав в UI)."""
    from core import tenant_users
    from core.tenant import tenant_store
    ...
    if not t:
        raise HTTPException(404, "Клиент не найден")
    return {"client_id": client_id, "tables": tenant_users.list_tenant_tables(t)}


@app.post(
    "/api/v1/admin/tenants/{client_id}/users", dependencies=[Depends(require_admin)], tags=["admin"]
)
def create_tenant_user_endpoint(client_id: str, payload: TenantUserCreateRequest):
    """Создаёт пользователя блока. Учитывает лимит max_users."""
    from core import tenant_users
    from core.tenant import tenant_store
    ...
    try:
        user = tenant_users.create_user(
            ...
            rls_filters=payload.rls_filters,
            can_dashboard=payload.can_dashboard,
            can_presentation=payload.can_presentation,
            max_users=t.max_users,
        )
    except ValueError as e:
        raise HTTPException(409, str(e))
    except Exception as e:  # noqa: BLE001
        raise HTTPException(500, f"Не удалось создать пользователя: {e}")
    return user


@app.patch(
    "/api/v1/admin/tenants/{client_id}/users/{username}",
    dependencies=[Depends(require_admin)],
    tags=["admin"],
)
def update_tenant_user_endpoint(client_id: str, username: str, payload: TenantUserUpdateRequest):
    """Обновляет пользователя блока (права, роль, RLS, пароль, статус)."""
    from core import tenant_users
    ...
    if not fields:
        raise HTTPException(400, "Нет полей для обновления")
    try:
        user = tenant_users.update_user(client_id, username, **fields)
    except Exception as e:  # noqa: BLE001
        raise HTTPException(500, f"Не удалось обновить пользователя: {e}")
    if not user:
...
def delete_tenant_user_endpoint(client_id: str, username: str):
    """Удаляет (мягко) пользователя блока."""
    from core import tenant_users

    ok = tenant_users.delete_user(client_id, username)
    if not ok:
        raise HTTPException(404, "Пользователь не найден")
    return {"success": True, "client_id": client_id, "username": username}
...
@app.get(
    "/api/v1/admin/prompts/{agent_name}", dependencies=[Depends(require_admin)], tags=["admin"]
)
...
@app.get("/api/v1/admin/rls", dependencies=[Depends(require_admin)], tags=["admin"])
def get_rls_rules():
    """Текущие RLS-правила + доступные роли и значения регионов для UI."""
    from app.auth import list_known_roles
    from core import rls

    rules: dict = {}
    try:
        rls.ensure_table()  # создаёт таблицу и сеет из YAML при первом открытии
        rules = rls.list_rules()
    except Exception as e:  # noqa: BLE001
        logger.warning("[RLS] ClickHouse недоступен, показываю YAML-фолбэк: %s", e)
        rules = rls.list_rules()

    roles = sorted(set(list_known_roles()) | set(rules.keys()))
    return {
        "status": "ok",
        "rules": rules,  # {role: {column: [values]}}
        "roles": roles,
        "regions": rls.list_region_values(),  # доступные значения колонки region
        "column": "region",
    }


class RlsRuleRequest(BaseModel):
    column: str = "region"
    values: list[str] = []


@app.put("/api/v1/admin/rls/{role}", dependencies=[Depends(require_admin)], tags=["admin"])
def set_rls_rule(role: str, payload: RlsRuleRequest):
    """Задать значения фильтра для роли (напр. region IN (...)). Применяется на лету."""
    from core import rls

    try:
        rls.set_role_filter(role, payload.column, payload.values)
    except ValueError as e:
        raise HTTPException(400, str(e))
    except Exception as e:  # noqa: BLE001
        raise HTTPException(500, f"Не удалось сохранить RLS-правило: {e}")
    return {"status": "ok", "role": role, "column": payload.column, "values": payload.values}
...
async def upload_workspace_file(
    ...
    """Загрузка файлов в персональный Workspace."""
    import os
    import shutil
    ...
    out_dir = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "..", "out", "workspaces", user.get("username", "guest")
        )
    )
    os.makedirs(out_dir, exist_ok=True)
    ...
    try:
        ...
        ch_client.command(
            f"CREATE TABLE IF NOT EXISTS {table_name} ({columns}) ENGINE = MergeTree() ORDER BY tuple()"
        )
...
async def pdf_analyze(
    ...
    try:
        ...
        if not text.strip():
            ...
            raise ValueError(
                "Не удалось извлечь текст из PDF. Возможно, документ отсканирован (содержит только изображения)."
            )
    ...
    try:
        ...
        class PDFMeta(PydanticBaseModel):
            ...
            summary: str = PydanticField(
                ..., description="Краткое содержание документа (2-3 предложения)"
            )
            key_topics: list[str] = PydanticField(..., description="5-7 ключевых тем документа")
            document_type: str = PydanticField(
                ..., description="Тип документа: отчет, инструкция, аналитика, презентация, и т.д."
            )
            suggested_questions: list[str] = PydanticField(
                ...,
                description="5 ключевых аналитических вопросов для презентации по этому документу",
            )
    ...
    if output_type == "dashboard":
        try:
            ...
            return {
                ...
                "dashboard_data": result.model_dump(mode="json")
...
def rag_search(payload: dict = Body(...)):
    ...
    try:
        ...
        chunks = [
            {"content": r[0][:400], "source": r[1], "score": round(float(r[2]), 4)} for r in rows
        ]
        return {
            "status": "ok",
            "context": context,
...
@app.post(
    "/api/v1/subscriptions/{sub_id}/toggle", dependencies=[Depends(require_admin)], tags=["admin"]
)
...
# ==========================================
# Schema, Semantic Rules & Dropzone
# ==========================================
...
def get_auto_insights():
    ...
    try:
        ...
        result = call_structured(
            prompt,
            schema=InsightsList,
            system="Ты умный налоговый аналитик. Пиши очень коротко и емко.",
        ...
        return {
            "status": "error",
            "insights": [{"type": "warning", "text": "Система аналитики временно недоступна."}],
        }
...
@app.get(
    "/api/v1/db/tables/{table}/data", dependencies=[Depends(require_admin)], tags=["workspace"]
)
def get_table_data(table: str, page: int = 1, limit: int = 50, search: str = ""):
    ...
    try:
        ...
        if search:
            ...
            if string_cols:
                conditions = " OR ".join(
                    [f"position(lower({col}), lower('{safe_search}')) > 0" for col in string_cols]
                )
...
@app.get(
    "/api/v1/db/tables/{table}/stats", dependencies=[Depends(require_admin)], tags=["workspace"]
)
...
@app.post(
    "/api/v1/knowledge/upload-text", dependencies=[Depends(require_admin)], tags=["workspace"]
)
async def upload_text_document(payload: dict = Body(...)):
    ...
    try:
        ...
        if data:
            ch_client.insert(
                "knowledge_base", data, column_names=["id", "source", "content", "embedding"]
            )
