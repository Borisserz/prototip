"""— admin-роутер ETL-оркестрации (кнопочная инициализация клиента).

маршруты (все под /api/v1/admin/tenants/{client_id}/...):
...
def _run_inline_etl(client_id: str) -> None:
    """Полный ETL клиента в текущем процессе (резервный вариант без Airflow)."""
...
def set_schedule(client_id: str, payload: ScheduleRequest):
    ...
    return {
        ...
        "note": (
            ...
            else "Синхронизировано с Airflow"
        ),
    }
