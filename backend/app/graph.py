"""Аналитический граф LangGraph: узлы, маршруты, сборка ответа."""
...
class GraphState(TypedDict):
    ...
    # накопленный state в процессе обработки
...
def memory_node(state: GraphState) -> dict:
    """Узел памяти: разовый RAG-поиск по истории за неделю для обогащения system prompt.
    При любой ошибке возвращает пустой контекст.
    """
    ...
    try:
        ...
        if ctx:
            logger.info(
                "[Memory Node] Контекст памяти собран для user_id=%s (%d симв.)", user_id, len(ctx)
            )
        else:
...
def planner_node(state: GraphState) -> dict:
    ...
    # RagAgent — получаем бизнес-контекст для планировщика
    rag = RagAgent()
    ...
    return {"business_context": context, "sub_questions": tasks}
...
def supervisor_node(state: GraphState) -> dict:
    ...
    # если есть drilldown — всегда роутим в data-агент, не в direct_answer
    if state.get("drilldown"):
        drilldown = state["drilldown"]
        filters = (
            getattr(drilldown, "filters", {})
            if hasattr(drilldown, "filters")
            else drilldown.get("filters", {})
        )
        logger.info(
            f"[Supervisor Node] Drilldown detected -> forcing route=data. Filters: {filters}"
        )
        # сбрасываем final_result — иначе route_after_supervisor сразу прыгнет в END
...
"""
    try:
        decision = call_structured(
            prompt,
            SupervisorDecision,
            system="Ты — маршрутизатор. Отвечай строго в JSON.",
            agent_name="supervisor_node",
        )
        logger.info(f"[Supervisor Node] Route decided: {decision.route}")
        if decision.route == "direct_answer":
            # direct_answer — упаковываем ответ LLM напрямую без агентов
...
def data_node(state: GraphState) -> dict:
    ...
    # drilldown-фильтры из стейта (если есть)
    ...
    with user_context(user_role):
        for idx, task in enumerate(sub_questions):
            ...
            # собираем DataAgentInput с фильтрами drilldown
            inp = DataAgentInput(
...
def analyst_node(state: GraphState) -> dict:
    ...
    # при drilldown уточняем вопрос аналитику, чтобы он дал конкретные выводы по фильтру
    ...
    if drilldown:
        ...
        getattr(drilldown, "dimension", "") if hasattr(drilldown, "dimension") else drilldown.get(
            "dimension", ""
        )
        getattr(drilldown, "segment_label", "") if hasattr(
            drilldown, "segment_label"
        ) else drilldown.get("segment_label", "")
        if filters:
    ...
    return {"analysis": res.reasoning, "raw_analysis_dict": analysis_dict}
...
def reviewer_node(state: GraphState) -> dict:
    ...
    if retries >= 1:  # максимум 1 попытка переработки
    ...
    if decision.is_good:
        ...
        return {"eval_feedback": decision.feedback, "eval_retry_count": retries + 1}
...
def presenter_node(state: GraphState) -> dict:
    ...
    if "прогноз" in q_lower:
        ...
        memory_result = {
            "title": chart_res.specs[0].title
            if chart_res.success and chart_res.specs
            else "Forecast",
            "charts": [
                {"chart_type": spec.chart_type, "title": spec.title} for spec in chart_specs
            ],
        }
        ...
        # передаём raw_data из стейта, чтобы DashboardAgent не ходил в БД повторно
        raw_data = state.get("raw_data", [])
        logger.info(f"[Presenter Node] Passing {len(raw_data)} rows of raw_data to DashboardAgent")
        # drilldown_filters передаём на случай, если агенту придётся перезапросить данные
        drilldown = state.get("drilldown")
        dd_filters = None
        if drilldown:
            if hasattr(drilldown, "filters"):
        ...
        req = DashboardRequest(
            question=state["question"], data=raw_data, max_charts=4, drilldown_filters=dd_filters
        )
        ...
        if dash_res.kpi_cards:
            ...
            for kpi in dash_res.kpi_cards:
                change_str = (
                    f" ({'+' if (kpi.change or 0) > 0 else ''}{kpi.change}% {kpi.change_period})"
                    if kpi.change
                    else ""
                )
...
def search_node(state: GraphState) -> dict:
    # если есть drilldown — пропускаем поиск: его final_result застрянет в route_after_supervisor
    if state.get("drilldown"):
        ...
        try:
            ...
            return {
                "final_result": AskResult(
                    ...
                    charts=[],  # передаём через reasoning
                )
            }
        except Exception as e:
...
def route_after_search(state: GraphState) -> str:
    # drilldown всегда идёт к планировщику, минуя кэш поиска
    if state.get("drilldown"):
        return "planner"
...
def route_after_supervisor(state: GraphState) -> str:
    # drilldown игнорирует протухший кэш из search_node — всегда в data
    if state.get("drilldown"):
        return "data"
    if state.get("final_result"):
        return "end"
...
def forecast_node(state: GraphState) -> dict:
    """узел предиктивной аналитики (numpy/scipy/statsmodels + LLM-резюме)."""
    from app.agent_context import emit_node_event
    ...
    if not res.success:
        return {
            "final_result": AskResult(
                question=state["question"],
                success=False,
                reasoning=f"Не удалось построить прогноз: {res.error}",
                charts=[],
            )
        }
...
def _instrument_node(name, fn):
    """оборачивает узел графа замером длительности (Prometheus)."""
    try:
...
def build_graph() -> StateGraph:
    ...
    workflow.add_conditional_edges("search", route_after_search, {"end": END, "planner": "planner"})

    workflow.add_edge("planner", "supervisor")
    workflow.add_conditional_edges(
        "supervisor", route_after_supervisor, {"end": END, "data": "data"}
    )
    ...
    workflow.add_conditional_edges(
        ...
        {"analyst": "analyst", "presenter": "presenter", "forecast": "forecast"},
    )
