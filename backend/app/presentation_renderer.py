...
FOOTER_TEXT = "Prototip BI  •  Analytics Platform  •  Министерство по налогам и сборам РБ"
BADGE_TEXT = "ДЛЯ СЛУЖЕБНОГО ПОЛЬЗОВАНИЯ | DRAFT"
...
class T:
    ...
    # цветовые акценты (0-5, циклически в луп-рендере)
    ACCENTS = [
        {
            "fg": RGBColor(124, 58, 237),
            "bg": RGBColor(30, 18, 60),
            "border": RGBColor(60, 35, 100),
            "light": RGBColor(167, 139, 250),
        },  # violet
        {
            "fg": RGBColor(59, 130, 246),
            "bg": RGBColor(18, 35, 65),
            "border": RGBColor(35, 65, 110),
            "light": RGBColor(147, 197, 253),
        },  # blue
        {
            "fg": RGBColor(16, 185, 129),
            "bg": RGBColor(10, 40, 30),
            "border": RGBColor(20, 70, 55),
            "light": RGBColor(110, 231, 183),
        },  # emerald
        {
            "fg": RGBColor(245, 158, 11),
            "bg": RGBColor(45, 30, 10),
            "border": RGBColor(80, 55, 20),
            "light": RGBColor(252, 211, 77),
        },  # amber
        {
            "fg": RGBColor(244, 63, 94),
            "bg": RGBColor(45, 10, 20),
            "border": RGBColor(80, 25, 40),
            "light": RGBColor(253, 164, 175),
        },  # rose
        {
            "fg": RGBColor(6, 182, 212),
            "bg": RGBColor(8, 35, 45),
            "border": RGBColor(15, 65, 80),
            "light": RGBColor(103, 232, 249),
        },  # cyan
    ]

    # unicode-иконки вместо SVG
    ICONS = {
        "pin": "◉",  # MapPin
        "cpu": "⊞",  # Cpu
        "target": "◎",  # Target
...
class PresentationRenderer:
    ...
    def _footer_bar(self, slide: Any, slide_num: int | None = None) -> None:
        self._rect(
            ...
            self.t.SLIDE_W,
            self.t.FOOTER_H,
            fill=self.t.FOOTER_BG,
            no_line=True,
        )
        self._rect(
            slide, 0, self.t.FOOTER_TOP, self.t.SLIDE_W, 0.005, fill=self.t.BORDER, no_line=True
        )
        self._textbox(
            slide,
            self.t.M,
            self.t.FOOTER_TOP + 0.09,
            10.0,
            0.28,
            FOOTER_TEXT,
            font_size=8,
            color=self.t.TEXT_DIMMED,
        )
    ...
    def _card(
        ...
        self._rect(
            ...
            width,
            height,
            fill=fill or self.t.CARD_BG,
            border=border or self.t.BORDER,
        )
    ...
    def _build_ranking_table_rows(self, res: AskResult, spec: ChartSpec) -> list[tuple[str, str]]:
        ...
        try:
            ...
            agg = spec.agg or "sum"
            g = (
                df.groupby(x_col, as_index=False)[y_col].mean()
                if agg == "mean"
            ...
            suffix = (
                "бел. руб."
                if any(k in y_col.lower() for k in ("debt", "accrued", "paid", "penalt"))
                else ""
            )
            return [
                (str(row[x_col]), format_number_ru(float(row[y_col]), suffix=suffix))
    ...
    def _render_ranking_table(
        ...
        *,
        ...
        self._style_table_cell(
            tbl.cell(0, 0),
            get_russian_label(spec.x).split(",")[0],
            bold=True,
            fill=self.t.TABLE_HEADER_BG,
            font_color=self.t.WHITE,
        )
        self._style_table_cell(
            tbl.cell(0, 1),
            f"{get_russian_label(y_col).split(',')[0]}, бел. руб.",
            bold=True,
            fill=self.t.TABLE_HEADER_BG,
            font_color=self.t.WHITE,
        )
        for i, (cat, val) in enumerate(rows_data[:5], start=1):
            ...
            self._style_table_cell(
                tbl.cell(i, 0),
                self.truncate_text(cat, 40),
                fill=fill,
                font_color=self.t.TEXT_PRIMARY,
            )
    ...
    def _render_right_analysis_panel(
        ...
        # иконка-лампочка (amber)
        self._icon_box(
            slide, left + pad, y + 0.12, 0.22, self.t.ICONS["bulb"], self.t.AMBER_BG, self.t.AMBER_L
        )
        self._textbox(
            slide,
            left + pad + 0.28,
        ...
        for label, value, symbol, ac in chips:
            ...
            self._icon_box(
                slide, left + 0.10, y + (chip_h - 0.22) / 2, 0.22, symbol, ac["fg"], self.t.WHITE
            )
            self._textbox(
                slide,
                left + 0.38,
    ...
    def render_slide_header(self, slide: Any, title: str, subtitle: str | None = None) -> None:
        ...
        badge_w = self._badge_tag(
            slide,
            self.t.M,
            0.27,
            "DATA",
            fill=self.t.EMERALD_BG,
            border=self.t.EMERALD_B,
            color=self.t.EMERALD_L,
        )
        self._textbox(
            slide,
            self.t.M + badge_w + 0.18,
            0.22,
            self.t.USABLE_W - badge_w - 0.25,
            0.58,
            title,
            font_size=font_size,
            bold=True,
    ...
    def _draw_chart_area(
        ...
        # белый фон карточки
        self._rect(
            slide, left, top, width, height, fill=RGBColor(255, 255, 255), border=self.t.BORDER
        )
        if png_path:
            ok = self._place_chart_image(
                slide, png_path, left=left, top=top, width=width, height=height
            )
        else:
            ok = False
        if not ok:
            # плейсхолдер внутри карточки
            self._textbox(
                slide,
                left + 0.5,
                top + height / 2 - 0.35,
    ...
    def _render_side_by_side_layout(
        ...
        self._draw_chart_area(
            slide, png_path, self.t.M, self.t.CONTENT_TOP, chart_w, self.t.CONTENT_H
        )
        self._render_right_analysis_panel(
            slide, res, right_left, self.t.CONTENT_TOP, right_w, self.t.CONTENT_H, chart_pref
        )
    ...
    def create_title_slide(self, slide: Any, *, date_str: str | None = None) -> None:
        ...
        # иконка логотипа Prototip BI
        self._icon_box(
            slide, m, 0.18, 0.40, self.t.ICONS["bar_chart"], self.t.VIOLET_BG, self.t.VIOLET_L
        )
        self._textbox(
            slide,
            m + 0.48,
    ...
    def create_summary_slide(
        self, slide: Any, overview: str, first_result: AskResult | None
    ) -> None:
        ...
        badge_w = self._badge_tag(
            ...
            "SUMMARY",
            fill=self.t.BLUE_BG,
            border=self.t.BLUE_B,
            color=self.t.BLUE_L,
        )
        self._textbox(slide, m + badge_w + 0.18, 0.18, 10.0, 0.58, "Обзор", font_size=24, bold=True)
        ...
        for ci, (lbl, val, symbol, ac) in enumerate(info_data):
            ...
            self._icon_box(
                slide, cx + 0.12, info_top + 0.22, 0.28, symbol, self.t.CARD_BG2, ac["light"]
            )
            self._textbox(
                slide,
                cx + 0.48,
            ...
            self._textbox(
                slide, cx + 0.48, info_top + 0.30, info_w - 0.58, 0.38, val, font_size=11, bold=True
            )
        ...
        kpi_params = [
            ...
            ("5 агентов", "Система", self.t.ICONS["activity"], self.t.ACCENTS[3]),
        ]
        chip_h = 0.70
        ry = rp_top + 0.24
    ...
    def create_themes_slide(self, slide: Any, themes: list[str], questions: list[str]) -> None:
        ...
        badge_w = self._badge_tag(
            ...
            0.22,
            "AGENDA",
            fill=self.t.VIOLET_BG,
            border=self.t.VIOLET_B,
            color=self.t.VIOLET_L,
        )
        self._textbox(
            slide, m + badge_w + 0.18, 0.18, 9.5, 0.58, "Темы и повестка", font_size=24, bold=True
        )
        self._textbox(
            slide,
            11.5,
        ...
        for i, theme in enumerate(themes[:n]):
            ...
            self._textbox(
                ...
                m + 0.54,
                cy + 0.22,
                self.t.USABLE_W - 1.30,
                card_h - 0.30,
                self.truncate_text(theme, 100),
                font_size=11,
        ...
        info_items = [
            ("Методология", "Text-to-SQL + BI", self.t.ICONS["cpu"]),
        ...
        for ci, (lbl, val, symbol) in enumerate(info_items):
            ...
            self._icon_box(
                slide, cx + 0.12, info_top + 0.14, 0.22, symbol, self.t.CARD_BG2, ac["light"]
            )
            self._textbox(
                slide,
                cx + 0.40,
    ...
    def render_question_slide(
        ...
        ctype = chart_pref or (
            getattr(res.chart_spec, "chart_type", None) if res.chart_spec else None
        )
        ...
        # футер/бейдж добавляется снаружи через build_question_slides_footer()
    ...
    def create_takeaways_slide(self, slide: Any, takeaways: list[str]) -> None:
        ...
        self._textbox(
            slide, m + badge_w + 0.18, 0.18, 9.5, 0.58, "Ключевые выводы", font_size=24, bold=True
        )
        self._textbox(
            slide,
            11.0,
        ...
        for i, tw in enumerate(shown):
            ...
            num_sz = 0.30
            self._icon_box(slide, cx + 0.12, cy + 0.12, num_sz, str(i + 1), ac["fg"], self.t.WHITE)
            # текстовые цвета content
        ...
        self._textbox(
            ...
            m + 0.46,
            bottom_top + 0.09,
            self.t.USABLE_W - 0.60,
            0.38,
            f"{n} выводов сформированы на основе автоматизированного анализа налоговых данных "
    ...
    def create_recommendations_slide(self, slide: Any, recommendations: list[str]) -> None:
        ...
        badge_w = self._badge_tag(
            ...
            0.22,
            "ACTIONS",
            fill=self.t.AMBER_BG,
            border=self.t.AMBER_B,
            color=self.t.AMBER_L,
        )
        self._textbox(
            slide, m + badge_w + 0.18, 0.18, 9.5, 0.58, "Рекомендации", font_size=24, bold=True
        )
        # Count badge
        cnt_txt = f"{len(recs)} рекомендаций"
        cnt_w = max(1.3, len(cnt_txt) * 0.10 + 0.2)
        self._rect(
            slide, 12.5 - cnt_w, 0.22, cnt_w, 0.30, fill=self.t.AMBER_BG, border=self.t.AMBER_B
        )
        self._textbox(
            slide,
            12.5 - cnt_w + 0.06,
        ...
        self._textbox(
            ...
            self.t.USABLE_W - 0.60,
            0.40,
            "Рекомендации сформированы автоматически. "
    ...
    def _add_centered_text(
        ...
        *,
        ...
        self._textbox(
            ...
            font_size=font_size,
            bold=bold,
            color=color or self.t.TEXT_PRIMARY,
            align=PP_ALIGN.CENTER,
        )
