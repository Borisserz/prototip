...
в ``app/main.py``). Поэтому отдельный маршрут здесь не нужен — достаточно
импортировать Модуль, чтобы метрики появились в выгрузке.
...
def observe_llm_call(
    *,
    ...
    status = status or "ok"
    try:
        LLM_CALL_DURATION.labels(agent=agent, model=model, status=status).observe(
            max(0.0, duration_s)
        )
