/**
 * Экспорт дашборда в PDF: рендерит HTML вне экрана, снимает через html2canvas, упаковывает в jsPDF.
 */
...
function buildExportHTML(data: any): string {
  ...
  const headerHTML = `
    <div style="background:${BG_CARD2};border-bottom:1px solid ${C_BORDER};padding:24px 40px;${flex(16)}align-items:center;">
      ...
      <div style="flex:1;padding-left:16px;">
        ...
        <div style="font-size:11px;color:${C_SLATE3};margin-top:4px;">Аналитический дашборд · ${now}</div>
      </div>
  ...
  const recHTML = recs.length ? `
    <div>
      <div style="font-size:10px;font-weight:700;color:${C_EMERALD};letter-spacing:1px;margin-bottom:12px;text-transform:uppercase;">Рекомендации</div>
      <div style="${flex(12)}flex-wrap:wrap;">
  ...
  const reasonHTML = reasoning ? `
    <div style="border-top:1px solid ${C_BORDER};padding-top:16px;${flex(12)}align-items:flex-start;">
      <div style="font-size:10px;font-weight:700;color:${C_SLATE5};letter-spacing:1px;text-transform:uppercase;white-space:nowrap;">Методология</div>
  ...
  const footerHTML = `
    <div style="background:${BG_CARD2};border-top:1px solid ${C_BORDER};padding:14px 40px;${flex(0)}align-items:center;justify-content:space-between;">
      ...
      <div style="font-size:10px;color:${C_PRIMARY};">prototip.bi</div>
    </div>`;
