...
export const SubscriptionsView: React.FC<SubscriptionsViewProps> = ({ onBackToChat }) => {
  ...
  const handleGeneratePreview = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!prompt.trim() || !verifiedEmail) return;

    setIsGeneratingPreview(true);
    // Имитация работы модульа для предпросмотра
  ...
  const handleSaveSubscription = async () => {
    ...
    const humanTitle = `${getFreqLabel()} в ${hour}:${minute} — ${prompt}`;
      ...
      <div className="h-full flex flex-col items-center justify-center p-8">
        ...
        >
          ...
          <p className="text-slate-400 mb-8 leading-relaxed text-sm">
            Введите ваш email, чтобы получать автоматические отчеты и дашборды, созданные по вашему расписанию.
          </p>
            ...
            >
              <div className="bg-slate-900/80 border border-slate-700/50 rounded-2xl p-6">
                <div className="border-b border-slate-700/50 pb-4 mb-4">
                  <div className="text-sm text-slate-400 mb-1">От: <span className="text-slate-300">Prototip BI &lt;reports@prototip.bi&gt;</span></div>
                  <div className="text-sm text-slate-400 mb-2">Кому: <span className="text-slate-300">{verifiedEmail}</span></div>
                  <div className="text-lg font-semibold text-white">Автоматический отчет: {prompt.substring(0, 40)}{prompt.length > 40 ? '...' : ''}</div>
                </div>
                
                <div className="prose prose-invert prose-sm max-w-none">
                  <p>Здравствуйте!</p>
                  <p>В соответствии с вашей подпиской, направляю актуальные данные по запросу: <i>"{prompt}"</i>.</p>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        <span className="flex items-center gap-1.5 text-xs text-slate-400 bg-slate-900/50 px-2 py-1 rounded-md border border-slate-700">
                          <Sparkles className="w-3.5 h-3.5 text-emerald-400" /> Отчет
                        </span>
                      </div>
                    </div>
