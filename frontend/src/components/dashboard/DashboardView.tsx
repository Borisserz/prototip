...
export const DashboardView: React.FC<DashboardViewProps> = ({ onBackToChat }) => {
      ...
      <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto h-full pb-10">
        ...
        <div className="flex-1 flex items-center justify-center border border-dashed border-white/10 rounded-2xl bg-white/5">
          <p className="text-slate-400 font-mono text-sm">
            DEBUG: dashboardData is NULL. activeDashboardId={activeDashboardId}
            <br />
            Is in history: {dashboardHistory.some(d => d.id === activeDashboardId) ? 'YES' : 'NO'}
            <br />
            History length: {dashboardHistory.length}
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar rounded-xl">
        ...
        {dashboardData.recommendations?.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            <h3 className="text-xs uppercase tracking-widest text-emerald-500 font-bold mb-4 ml-1">Рекомендации</h3>
          ...
          {dashboardData.reasoning && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="mt-8 pt-6 border-t border-white/5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-slate-400 mb-1">Методология (Reasoning)</h4>
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400 mt-1">
                    Вы уверены, что хотите удалить Новый Дашборд? Это действие нельзя отменить.
                  </p>
                </div>
              </div>
