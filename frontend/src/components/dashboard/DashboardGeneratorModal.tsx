  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data?.charts && data.charts.length > 0) {
        const fullDashboard = {
          title: data.title || "Новый Дашборд",
          summary: data.summary || "Анализ на основе ваших данных.",
      ...
      console.error(e);
      setErrorMessage("Произошла системная ошибка при подготовке дашборда.");
      setStatus('ERROR');
    }
  };
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-white tracking-wide">
                    {status === 'IDLE' && 'Генерация Дашборда'}
                    {status === 'GENERATING' && 'Подготовка данных...'}
                    {status === 'ERROR' && 'Ошибка подготовки'}
                  </h2>
                  <p className="text-xs text-slate-400">
                    {status === 'IDLE' && 'Автоматическое создание набора графиков по теме'}
                    {status === 'GENERATING' && 'Ожидайте, система готовит данные и визуализации'}
