...
export const DynamicChart: React.FC<DynamicChartProps> = ({ content, onPin, isPinnedView = false, onChartClick }) => {
  ...
  const handleChartElementClick = (key: string, value: any, _e?: any) => {
    // Always open context menu regardless of event type (recharts may pass synthetic events without clientX)
  ...
  try {
    // Use trim() to remove leading/trailing whitespace (including newlines from ```json\n...\n``` blocks).
    // Do NOT use replace(/\n/g, '\\n') — it converts structural JSON whitespace into invalid escape sequences.
    // Python's json.dumps already properly escapes newlines within string values as \\n.
    ...
    if (parsed && typeof parsed === 'object' && parsed.chart_type && parsed.data) {
      ...
      const renderChart = (expanded = false) => {
        ...
        // Aggregate data by xKey to prevent messy charts from granular raw_data
        ...
        const renderChartVisual = (expanded = false) => {
          ...
          if (isBar) {
            ...
            // Prepare data for treemap (name and size are required)
                  ...
                  >
                    ...
                    <Button 
                      ...
                      onClick={() => { 
                        ...
                        // Extract meaningful string value if it's an object (e.g. from pie chart entry)
                    ...
                    >
                      <Info className="w-4 h-4 mr-2 text-primary" /> Детали (Drill-down)
                    </Button>
                    <Button 
                      variant="ghost" 
                      className="w-full justify-start text-sm text-slate-300 hover:text-white hover:bg-slate-800 h-9"
