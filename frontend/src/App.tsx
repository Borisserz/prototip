...
import { LayoutDashboard, FolderOpen, MessageSquare, X, UploadCloud, Sparkles, AlertTriangle, CheckCircle2, ShieldAlert, Presentation, Database } from "lucide-react"
...
import { AIDashboardView } from "./components/dashboard/AIDashboardView";
...
export default function App() {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      setMessages([{ role: "assistant", content: "Приветствую! Я ваш AI-аналитик. Готов предоставить данные, построить графики или ответить на бизнес-вопросы." }])
    }
  }, [])
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden relative">
                        ...
                        {dashboardHistory.map(dashboard => (
                          <div 
                            ...
                            className={`w-full p-3 rounded-lg flex items-center justify-between transition-colors border cursor-pointer group ${
                              activeDashboardId === dashboard.id && mainView === 'ai_dashboard' 
                                ? 'bg-emerald-500/10 border-emerald-500/30' 
                                : 'bg-slate-800/40 hover:bg-slate-800 border-slate-700/50'
                            }`}
                            onClick={() => {
                              setActiveDashboard(dashboard.id);
                              setMainView('ai_dashboard');
                            }}
                          >
                            <div className="flex items-center gap-3 overflow-hidden">
                              <LayoutDashboard className={`w-4 h-4 flex-shrink-0 transition-transform group-hover:scale-110 ${
                                activeDashboardId === dashboard.id && mainView === 'ai_dashboard' ? 'text-emerald-400' : 'text-slate-400 group-hover:text-emerald-400'
                              }`} />
                              <div className="flex flex-col overflow-hidden">
                                <span className={`text-sm font-medium truncate ${
                                  activeDashboardId === dashboard.id && mainView === 'ai_dashboard' ? 'text-emerald-400' : 'text-slate-200'
      ...
      <div className="flex-1 flex flex-col min-w-0 z-0">
        ...
        {/* Generator Modals */}
        <DashboardGeneratorModal isOpen={isDashModalOpen} onClose={() => setDashModalOpen(false)} onSuccess={() => setMainView('ai_dashboard')} token={token} />
